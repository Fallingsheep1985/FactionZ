...

{
	...
	if (!(_x in _RemovedPartsArray)) then {
		...
		if (_damage < 0.10) then {
			...
			_string = format[("<t color=""#007ab7"">" + ("Salvage") +"</t>",_cmpt,_damagePercent];
			_string = format["<t %1>%2</t>",_color,_string]; //Remove - Part
			_handle = dayz_myCursorTarget addAction [_string, "sheep\salvage.sqf",[_vehicle,_part,_x], 0, false, true, "",""];
...
if (count _hitpoints > 0 ) then {
	//ArmA OA String
	_cancel = dayz_myCursorTarget addAction [localize "str_action_cancel_action", "sheep\repair_cancel.sqf","repair", 0, true, false, "",""];
