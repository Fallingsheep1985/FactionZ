...
_Redbull = {_x == "ItemSodaMdew"} count magazines player;

if ((_Jerrycan >= 1)&&(_Redbull >= 1)) then {
	player removeMagazine "ItemJerrycan";
	player removeMagazine "ItemSodaMdew";
	...
	_msg = "You need: "+str(1 - _Jerrycan)+"x(Full Jerry Can) and "+str(1 - _Redbull)+"x(Mountain Dew Soda) to install nitro boost.";
	cutText [_msg, "PLAIN DOWN"];
};