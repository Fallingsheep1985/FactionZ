...

{
	...
	if (_damage > 0) then {
		...
		_cmpt = format[("<t color=""#007ab7"">" + ("Repair") +"</t>",_cmpt,_damagePercent];

		_string = format["<t %1>%2</t>",_color,_cmpt]; //Repair - Part
		_handle = dayz_myCursorTarget addAction [_string, "sheep\repair.sqf",[_vehicle,_part,_x], 0, false, true, "",""];
